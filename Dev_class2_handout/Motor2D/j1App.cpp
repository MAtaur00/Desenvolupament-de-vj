...
bool j1App::Awake()
{
	...
	xml_parse_result result = pconfig.load_file("config.xml");

	/*cout << "Load result: " << result.description() << " , mesh name: " <<
		pconfig.child("mesh").attribute("atrib").value() << endl;*/

	// If everything goes well, load the top tag inside the xml_node property
	// created in the last TODO

	if (result)  //també es pot fer   if (pconfig.load_file("config.xml"))
	{
		cout << "XML parsed without errors" << endl;
		read = pconfig.child("config");
	}

	else {
		cout << "XML parsed with errors" << endl;
		cout << "Erros description: " << result.description() << endl;
	}
