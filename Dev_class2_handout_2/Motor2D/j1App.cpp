...
bool j1App::LoadFile()
{
	...
	pugi::xml_parse_result result = savesdoc.load_file("savegame.xml");
...
void j1App::Load()
{
	...
	for (item = modules.start; item != NULL; item = item->next)
	{
		...
		item->data->Load(&savenode.child(item->data->name.GetString()));
	}
}
...
void j1App::Save() const
{
	...
	for (item = modules.start; item != NULL; item = item->next)
	{
		...
		item->data->Save(&savenode.child(item->data->name.GetString()));
	}

	savesdoc.save_file("savegame.xml");
}
